...
public class QueryDaoImplementation implements QueryDao{
    ...
    private String QUERY_FIND_QUERY_BY_NAME = "Select q from QueryEntity q where q.name = :name";
    ...
    public QueryEntity find(String queryEntityName) {
        ...
        System.out.println("NAME TO LOOK FOR IN DAO "+queryEntityName);
        List<QueryEntity> queryEntityList = entityManager.createQuery(QUERY_FIND_QUERY_BY_NAME)
                .setParameter("name", queryEntityName)
                .getResultList();

        System.out.println("Queries result list size "+queryEntityList.size());
