...
public class TvSeriesEntity {
    ...
    @OneToMany(targetEntity = EpisodeEntity.class, mappedBy="tvSeries", cascade = CascadeType.ALL)
    ...
    public TvSeriesEntity() {
        episodes = new ArrayList<EpisodeEntity>();
        actors = new ArrayList<ActorEntity>();
    ...
    /**
     * This is used for merging the results coming from the three servers.
     *
     * The comparison is made by firstly comparing the tv rage id, imdb id and then by title.
     * If at least one pair of values is identical, the two TvSeriesEntity objects are considered as
     * being equal as well.
     * @param o
     * @return
     */
